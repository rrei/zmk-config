...
5-column split keyboard with nice_nano_v2 controllers. The configuration implements
advanced features like mouse emulation, multi-layer typing with home row mods, and
...
- Build targets defined in `build.yaml`: `nice_nano_v2` with `corne_left` and
  `corne_right` shields
...
- **Combos**: Extensive combo system for common keys (TAB, SPACE, ENTER, BACKSPACE,
  etc.)
...
6. Updated `diagrams/corne.svg` diagram will be automatically committed if keymap
   changed
...
- Mouse movement speed controlled via `ZMK_POINTING_DEFAULT_MOVE_VAL` and
  `ZMK_POINTING_DEFAULT_SCRL_VAL`
- Precision layer uses `zip_xy_scaler` and `zip_scroll_scaler` with 1:3 ratio for fine
  control
